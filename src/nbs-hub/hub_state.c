...
int hub_save_manifest(hub_ctx *ctx)
{
    ...
    fprintf(fp, "# NBS Hub Manifest\n");
    fprintf(fp, "# Auto-generated by: nbs-hub\n\n");
    fprintf(fp, "project_dir=%s\n", m->project_dir);
    fprintf(fp, "terminal_goal=%s\n", m->terminal_goal);
    fprintf(fp, "workers_dir=%s\n", m->workers_dir);
    fprintf(fp, "hub_dir=%s\n", m->hub_dir);
    fprintf(fp, "\n# Document registry\n");
...
int hub_load_state(hub_ctx *ctx)
{
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (strcmp(key, "phase") == 0)
            ...
            s->total_prompts = atoi(value);
        else if (strcmp(key, "workers_completed") == 0)
            s->workers_completed = atoi(value);
        else if (strcmp(key, "workers_since_check") == 0)
            s->workers_since_check = atoi(value);
        else if (strcmp(key, "audit_required") == 0)
            s->audit_required = atoi(value);
            ...
            s->stall_threshold_seconds = atoi(value);
    }
...
int hub_save_state(hub_ctx *ctx)
{
    ...
    fprintf(fp, "# NBS Hub State\n");
    fprintf(fp, "# Updated atomically by hub commands\n\n");
    fprintf(fp, "phase=%d\n", s->phase);
    fprintf(fp, "phase_name=%s\n", s->phase_name);
    fprintf(fp, "phase_gate_criteria=%s\n", s->phase_gate_criteria);
    fprintf(fp, "total_prompts=%d\n", s->total_prompts);
    fprintf(fp, "workers_completed=%d\n", s->workers_completed);
    fprintf(fp, "workers_since_check=%d\n", s->workers_since_check);
    fprintf(fp, "audit_required=%d\n", s->audit_required);
    fprintf(fp, "last_audit_time=%s\n", format_time(s->last_audit_time));
    fprintf(fp, "last_spawn_time=%s\n", format_time(s->last_spawn_time));
    fprintf(fp, "stall_threshold_seconds=%d\n", s->stall_threshold_seconds);
