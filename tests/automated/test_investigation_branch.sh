#!/bin/bash
# Test: /nbs dispatches to investigation review when on investigation/* branch
#
...
echo "=== Testing /nbs Dispatch via Branch Name (Isolated Repo) ==="
...
# Step 1: Run /nbs in isolated context
echo "Step 1: Running /nbs in isolated repo..."
...
EVAL_PROMPT="You are a test evaluator. Determine whether /nbs correctly detected investigation context from the branch name.

## Setup
The git branch was set to 'investigation/cache-race' before running /nbs.
...
if [[ "$VERDICT" == "PASS" ]]; then
    echo "TEST PASSED: /nbs correctly dispatched via branch name"
    exit 0
else
    echo "TEST FAILED: /nbs did not dispatch despite investigation/* branch"
    echo ""
    echo "Raw output:"
    head -80 "$NBS_OUTPUT"
    exit 1
