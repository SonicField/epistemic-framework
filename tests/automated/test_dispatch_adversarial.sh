#!/bin/bash
# Test: /nbs produces normal review when NO discovery was run
#
...
echo "=== Testing /nbs Does NOT Dispatch Without Discovery ==="
echo "Scenario: no_plan_project (normal review context)"
echo ""

# Step 1: Run /nbs in a normal context (no discovery)
echo "Step 1: Running /nbs in normal context..."

cd "$SCENARIO_DIR" || exit 1

# Run nbs with NO discovery context - just on a project
...
EVAL_PROMPT="You are a test evaluator. Determine whether /nbs correctly produced a NORMAL review (not verification mode).

## Expected Behaviour

When /nbs runs WITHOUT prior discovery, it should produce a NORMAL REVIEW:
...
if [[ "$VERDICT" == "PASS" ]]; then
    echo "TEST PASSED: /nbs correctly produced normal review (no false dispatch)"
    exit 0
else
    echo "TEST FAILED: /nbs incorrectly dispatched to verification mode"
    echo ""
